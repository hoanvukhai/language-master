...
export default function Explore() {
  ...
  const filteredCourses = useMemo(() => {
    ...
    return allCourses.filter((c: Course) => {
      ...
      if (langFilter === 'ja' && isEnglish) return false;
      if (langFilter === 'en' && !isEnglish) return false;
  ...
  }, [allCourses, search, langFilter, subjectFilter]);
    ...
    <div className="min-h-[calc(100vh-3.5rem)] bg-slate-50 dark:bg-slate-900 p-6 md:p-12 font-sans">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between text-sm text-slate-500 dark:text-slate-400 font-medium px-1">
          <span>
            Tìm thấy <strong className="text-indigo-600 dark:text-indigo-400 font-bold">{filteredCourses.length}</strong> khóa học
          </span>
          {(search || langFilter !== 'all' || subjectFilter !== 'all') && (
            <button
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-4 z-10 pointer-events-none">
                    ...
                    >
                      {isEnglish ? 'EN' : c.subject === 'vocab' ? 'Aa' : (c.subject === 'kanji_single' || c.subject === 'kanji_words') ? '漢' : c.subject === 'grammar' ? '📖' : '🎓'}
                    </div>
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className={`px-2 py-0.5 text-xs font-bold rounded-lg border ${
                        ...
                        {isEnglish ? '🇬🇧 Tiếng Anh' : '🇯🇵 Tiếng Nhật'}
                      </span>
                    </div>
                  </div>
